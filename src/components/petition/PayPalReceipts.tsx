import { PAYPAL_COUNTRY_ROWS } from "@/data/paypal-petition";

const ACCENT = "#1F6CFF";

export default function PayPalReceipts() {
  const max = Math.max(...PAYPAL_COUNTRY_ROWS.map((r) => r.pop));
  return (
    <section
      id="receipts"
        ...
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-10 lg:gap-12 items-end mb-14 lg:mb-[72px]">
          <div>
            ...
            >
              ...
              YES.
              <br />
              <span style={{ color: ACCENT }}>US. NO.</span>
            </h2>
          </div>
          <div className="text-[16px] sm:text-[17px] text-grey-6 leading-[1.55] space-y-4 max-w-[640px]">
            <p>
              Kosovo borders <strong className="text-paper font-bold">five countries</strong>.
              PayPal serves <strong className="text-paper font-bold">all five</strong> — send,
              receive, and business accounts.
            </p>
            <p>
              The technical, legal, and AML/KYC work has been done five times for our neighbours.{" "}
              <strong className="text-paper font-bold">
                This is a market-entry decision, not a feasibility one.
              </strong>
            </p>
          </div>
        </div>
            ...
            return (
              <div
                ...
                className={`relative grid grid-cols-[40px_1.2fr_2fr_auto] md:grid-cols-[60px_1.2fr_2.5fr_auto] gap-4 md:gap-6 items-center py-5 md:py-7 border-b border-rule ${
                  row.us ? "bg-[rgba(31,108,255,0.04)]" : ""
                }`}
              >
                <div
                ...
                <div className="relative h-9 flex items-center">
                  ...
                  >
                    <span className="font-extrabold text-[15px]">{row.pop.toFixed(1)}M</span>
                ...
                {row.us && (
                  <div className="col-span-4 text-[11px] tracking-[0.14em] uppercase text-grey-5 font-bold mt-2 inline-flex items-center gap-3">
                    ...
                    <span>Bordered on every side. Not on the list.</span>
                  </div>
                )}
              </div>
            );
