import { SPOTIFY_RECEIPT_ROWS } from "@/data/spotify-petition";

const ACCENT = "#00C46A";

export default function SpotifyReceipts() {
  const max = Math.max(...SPOTIFY_RECEIPT_ROWS.map((r) => r.sp));
  return (
    <section
      id="receipts"
    ...
    >
      <div className="wrap">
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-10 lg:gap-12 items-end mb-14 lg:mb-[72px]">
          <div>
            ...
            >
              ...
              SMALLER THAN US
              <br />
              <span style={{ color: ACCENT }}>GETS THE APP IN THEIR TONGUE.</span>
            </h2>
          </div>
          <div className="text-[16px] sm:text-[17px] text-grey-6 leading-[1.55] space-y-4 max-w-[640px]">
            <p>
              Spotify&apos;s localization team shipped{" "}
              <strong className="text-paper font-bold">Icelandic (350K speakers)</strong> in 2021.
              They added <strong className="text-paper font-bold">four more</strong> Eastern
              European languages by 2023.
            </p>
            <p>
              Albanian —{" "}
              <strong className="text-paper font-bold">6× larger than Bosnian</strong>, with
              millions of paying users across Tirana, Pristina, Skopje, Zürich, Berlin and New York
              — is still missing from the language picker.
            </p>
          </div>
        </div>

        <div className="border-t border-rule">
          {SPOTIFY_RECEIPT_ROWS.map((row, i) => {
            const pct = (row.sp / max) * 100;
            return (
              <div
                key={i}
              ...
              >
                ...
                {row.us && (
                  <div className="col-span-4 text-[11px] tracking-[0.14em] uppercase text-grey-5 font-bold mt-2 inline-flex items-center gap-3">
                    ...
                    <span>Largest speaker base. Zero support.</span>
                  </div>
                )}
              </div>
            );
        ...
        <div className="mt-8 text-[11px] text-grey-5 tracking-[0.04em] flex items-center gap-2.5 flex-wrap">
          ...
          Speaker counts: Ethnologue (2024). Spotify language support:
          spotify.com/account/profile · audited Apr 2026.
        </div>
      </div>
    </section>
  );
