import { UBER_RECEIPT_ROWS } from "@/data/uber-petition";

const ACCENT = "#F6F600";

export default function UberReceipts() {
  const max = Math.max(...UBER_RECEIPT_ROWS.map((r) => r.pop));
  return (
    <section
      id="receipts"
        ...
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-10 lg:gap-12 items-end mb-14 lg:mb-[72px]">
          <div>
            ...
            >
              ...
              EVERY CAPITAL.
              <br />
              <span style={{ color: ACCENT }}>EVERY MARKET TIER.</span>
            </h2>
          </div>
          <div className="text-[16px] sm:text-[17px] text-grey-6 leading-[1.55] space-y-4 max-w-[640px]">
            <p>
              Uber operates in <strong className="text-paper font-bold">Tirana</strong> (550K
              people, smaller GDP per capita than Pristina). It does not operate here.
            </p>
            <p>
              Pristina has airport-to-city demand. Diaspora summer demand. Conference demand.{" "}
              <strong className="text-paper font-bold">
                The unit economics aren&apos;t the question — the launch decision is.
              </strong>
            </p>
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="relative h-9 flex items-center">
                  ...
                  >
                    <span className="font-extrabold text-[15px]">{row.pop.toFixed(2)}M</span>
                ...
                {row.us && (
                  <div className="col-span-4 text-[11px] tracking-[0.14em] uppercase text-grey-5 font-bold mt-2 inline-flex items-center gap-3">
                    ...
                    <span>Capital city. Zero ride product.</span>
                  </div>
                )}
              </div>
            );
